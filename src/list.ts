...
import { ArrayOf } from "./melange-ffi";
...
export const List = {
   ...
   */
  ofArray<T>(array: ArrayOf<T>[]): List<T> {
   ...
   */
  at<T>(index: number, list: List<T>): Result<T, string> {
    try {
   ...
   */
  find<T>(predicate: (value: T) => boolean, list: List<T>): Result<T, string> {
   ...
   */
  map<T, U>(fn: (value: T, index: number) => U, list: List<T>): List<U> {
   ...
   */
  filterMap<T, U>(fn: (value: T) => U | null, list: List<T>): List<U> {
